...
*(Succeeded by `ED1` / `LIVINGHOME2`, 2026-07-22: this **two-register** rule of thumb is superseded by the **three-register classification test** (`LIVING_HOME_ENVIRONMENTAL_DRESSING_ARCHITECTURE.md` § 4.3), which inserts a third register — **Environmental Dressing** — between House and Household Life. Its *purpose* survives strengthened, not weakened: nothing may dress the **house** while pretending to be the household's **life**, and the empty-house test still binds the Life register in full. What the third register adds is one narrow, owned class the old two-register wording could not name: a **claim-free object owned by the home** (a bowl of apples, pumpkins by the door) that reads as neither house architecture nor household data — permitted to be present for an empty household precisely because it asserts nothing about them. It enters **only** through that owned test, **item-by-item**, and is **DECLARED-NOT-BUILT**.)*
...
- **LH3 — A tradition never changes the house.** No dressing, no theme, no palette, no motion, no environment change, for any occasion, at any participation level. A tradition's whole expression is **words** (the Companion's), **doors** (what is aimed), and **food** (what is suggested) — the one-morning and one-season laws (Blueprint § 7/§ 6.1, cited) admit no occasion exception, and the shell's constancy earns no seasonal-moment exception (`THA_EXPERIENCE_LANGUAGE.md` § 5, cited). *(Refined by `ED1` / `LIVINGHOME2`, 2026-07-22: LH3's core is **unchanged and permanent** — a tradition **never changes the House** (orchard, light, walls, palette, layout, navigation: no theme, no weather, no environment change, ever). What is refined is only the word "dressing": a tradition may reach the **registered Environmental Dressing layer** (`LIVING_HOME_ENVIRONMENTAL_DRESSING_ARCHITECTURE.md` § 7.2) — a claim-free object the home sets on its own surface, e.g. one simple wreath — **solely through the household's explicit, per-tradition dressing permission** (the switch added below § 7.3), which is **OFF by default** and never suggested, pre-ticked, inferred, or assumed. Dressing is **not** the House and never changes it; it is the home's own hospitality, identical for every household that permits it. **DECLARED-NOT-BUILT**: celebration dressing is LIVINGHOME2 Phase 5, hard-blocked, and no wreath ships until its full gate stack and LH7 privacy review pass.)*
...
*(Amended by `ED1` / `LIVINGHOME2` § 7.2, 2026-07-22 — the ladder's three rungs are **unchanged**: their count and meaning stay governed here (LH2), and **no rung changes the House or permits dressing.** The Environmental Dressing permission that `LIVINGHOME2` § 7.2.2 reserved to this amendment is added as a **separate, per-tradition dressing switch — distinct from the participation ladder, OFF by default — not a fourth rung.** Reasoning recorded for the owner's review: dressing consent is categorically different from participation — it can render belief **visually**, engaging LH7's Article 9 special-category care — so it belongs on its own fail-closed axis and must not renumber the governed rungs. The switch, when on, grants only a **registered, universal** dressing item (the same simple wreath for everyone) bound to a **declared** occasion (LH1) at the household's explicit leave; default is nothing, and a household that declares nothing — or permits nothing — sees a home with **no occasion in it, completely and honestly**. **DECLARED-NOT-BUILT**: no celebration dressing is lawful until `LIVINGHOME2` Phase 5's gate stack and the LH7 privacy review pass.)*
...
4. **No tradition ever carries an asset.** LH3 stated as asset law: the Traditions & Celebrations domain owns words and dates, never imagery, iconography, or decoration. There is no lawful future in which a declared occasion admits a themed asset — that future would require amending the one-season law itself, which this document does not propose and records as the cost any such proposal must name (`NORTH2` § 3.5 pattern, cited). *(Refined by `ED1` / `LIVINGHOME2`, 2026-07-22 — restated as: **no tradition ever changes the House** (orchard, light, walls: absolute, unchanged above), **and** a tradition reaches the **registered Environmental Dressing layer only through the household's explicit, per-tradition dressing permission** (§ 7.3 switch; `LIVINGHOME2` § 7.2). The clause's original purpose — no themed asset arriving by taste, by default, or by inference — survives verbatim: a registered dressing item is never themed-per-household, never generated, never inferred, and enters one at a time by admission. **The cost this item said any such proposal must name is now named and paid deliberately by the owner** (`LIVINGHOME2` User Acceptance Evidence), not eroded — and the price stayed low because the **one-season law is untouched**: the year enters as **objects through the door**, never as **weather on the orchard**. **DECLARED-NOT-BUILT**.)*
