# THA Companion Notice Engine Architecture

> **Re-homed by OBS1 (2026-07-08):** this document was established by INT20 as "THA Observation Engine Architecture". The Observation Engine name now designates the Intelligence Platform's canonical telemetry service (`THA_OBSERVATION_ENGINE_ARCHITECTURE.md`); this ambient-notice component continues unchanged in meaning under its new name, the Notice Engine. The telemetry Observation Engine and this Notice Engine are disjoint: notices are user-facing facts about the household's own data; observations are operator-facing telemetry about platform execution.
...
**There is one Notice Engine. Every passive notice — every "here is something true about your own data that you did not ask about" — is a Notice: a fact an existing owner already computed, adapted into one shape, selected and rate-limited under one attention budget, voiced by the one Behaviour Engine, and, when it proposes action, resolved through the one opportunity-delivery lifecycle. No other component may decide what the platform proactively notices, how often, or in what order.**
...
> The Notice Engine may change **which already-true fact** is surfaced, and **when**. It may never change **what is true, what is permitted, or what requires confirmation** — and it may never compute a new metric, threshold, or judgement of its own.
...
THA already has every part of a notice pipeline — built, tested, and owned in exactly one place each:
...
- **The adapter** exists. `server/intelligence/conversation/notice-engine.ts` (EWX1) wraps those already-computed facts into one `Notice` shape across seven closed categories and applies the Silence Rules — de-dupe, priority order, at most two per moment.
...
What THA does **not** have is a single governed answer to *"what does the platform notice, and who is allowed to tell the user?"* On the current branch the canonical chain is **dormant** — no server route exposes notices, food opportunities, or learning signals, and no page renders the panels — while **parallel, ungoverned notice channels are live**: `GET /api/home/intelligence` and `GET /api/planner/weeks/:weekId/intelligence` assemble `{ celebration, seasonalHighlight, opportunity, householdInsight }` directly from the `shared/discovery`, `shared/stories`, and `shared/seasonal` engines, and the WX7 pantry-opportunities block builds its own "opportunities" in `server/routes.ts` — none of it passing through OD1's mute/de-dupe/lifecycle governance or the Silence Rules.
...
**The symmetry worth stating once:** the Context Composition Engine budgets the *model's* attention — every byte the LLM reads as grounding, under one budget. The Notice Engine budgets the *user's* attention — every unprompted notice the user reads, under one budget. Same architectural move, opposite direction, and the two never overlap: the ambient notice seam never touches the LLM, and the per-turn prompt is never assembled by the Notice Engine (§5.3).
...
A **Notice** is a passive, request-scoped projection of a fact an existing owner already computed about **this user's or household's own data**. It has a closed category, a priority, and a fact payload copied **verbatim** from its producer. It is computed fresh on every request and persisted nowhere.

### 2.2 The closed source taxonomy

Seven categories (`NoticeCategory`), five fact kinds, each with exactly one producing owner. The engine adds no eighth category and no new fact without a registered owner behind it — an unmapped domain is dropped honestly, never guessed into a category.
...
- **No model-inferred patterns.** Noticing is deterministic adaptation. Anything learned belongs to EL1, behind its ≥3-events / ≥0.7-consistency / household-confirmation gates. **A single notice never becomes a preference** — that is EL1's founding rule, restated here as the Notice Engine's boundary with learning.
- **No raw behaviour surveillance.** Producers notice *state* the user already owns (their planner, pantry, shopping list, diary-derived trends), not interaction telemetry. Guidance analytics and product-event logging remain separately owned and are not notice sources.
...
| The Notice itself (category, priority, verbatim fact) | **Transient.** The Notice Engine composes it per request and owns no store | **Never persisted** |
| The attention budget (Silence Rules: de-dupe, priority order, cap 2) | **The Notice Engine — its only genuine ownership** | n/a (pure function) |
| The category → interaction-kind vocabulary | `shared/companion-interaction.ts` (closed, shared verbatim server↔client) | n/a |
| The phrasing of a voiced notice | The Behaviour Engine (`phraseNotice`) — the same one voice seam every Companion output uses | n/a |
...
1. **The Notice Engine owns a decision, not data.** Its single canonical possession is the Silence Rules — the un-bypassable choke point deciding volume and order of ambient notices. Everything else it touches is someone else's, verbatim.
2. **Statelessness is the current design, and its known cost is already named.** CPA1 §11 G6 records that without a cross-session log, a notable notice can repeat across sessions. The named (not built) fix — a small additive `companion_observation_log` (name predates the OBS1 re-home; it would be re-titled at build time) — would, if ever approved, record *delivery of a notice*, never a copy of any producer's fact, and would be owned by the Notice Engine's store exactly as `opportunity_deliveries` is owned by OD1's. That remains its own future Rule 8 review.

---

## 4. FROM NOTICE TO RECOMMENDATION

A notice becomes a recommendation by **graduation, never by escalation** — each step crosses into a mechanism that already exists, already has an owner, and already gates it:
...
```
  ...
  opportunity     OD1 collects via intelligencePlatform.handle(), filters
   ...
   ▼
  notice          the Notice Engine adapts what survives — verbatim
   │              explanation and suggestedAction — and the Silence Rules
   │              choose at most two per moment
   ▼
  voiced notice   the Behaviour Engine voices it; the Companion panel or a
...
- **A recommendation is always the producer's, verbatim.** The Notice Engine forwards `explanation` and `suggestedAction` unchanged (asserted in its test suite). It never composes advice, never re-ranks a producer's judgement, never merges two producers' facts into a new claim.
- **No notice executes anything.** Accepting an opportunity transitions a delivery record; it does not touch a planner entry, shopping item, or pantry row. Acting requires a separate, confirmed intent whose confirmation tier is decided server-side by capability class (TIP2 §4–5) — a notice cannot lower it.
- **Learning is downstream of outcomes, never of notices.** Showing a notice generates no evidence. Only what the user *does* is evidence, and only EL1's thresholds turn evidence into a signal, and only the household's explicit confirmation turns a signal into something any owner may act on.
- **Silence is a first-class outcome.** Most true facts should never be surfaced. The cap of two per moment, the notability gates (×7 streaks, ×10 diversity), and the mute list mean the default disposition of a noticeable fact is *unspoken* — the engine exists as much to withhold honestly as to surface.
...
The Notice Engine sits entirely on the read side. It invokes nothing, and nothing about a notice shortcuts the intent pipeline:

- Resolving a notice-backed opportunity uses OD1's registered verbs (`review`/`approve`/`delete`) — three of the closed 20, platform-wide strong-confirmation, no special case.
...
- No new verb, no new capability class, and no confirmation-tier exception may ever be introduced *for* notices.

### 5.2 With the Capability Registry (TIP2)

The Notice Engine is **not a capability and must not become one.** It is a Companion-layer presentation seam (CPA1 §7: "the Companion Platform registers zero new capabilities"), and it holds no reference to the registry. The registered capabilities in this pipeline are `opportunity-delivery` and `evidence-learning`; every producer read flows through `intelligencePlatform.handle()` **performed by the calling route, not by the engine** — the engine receives plain data and stays a zero-I/O pure module. Widening what is noticeable therefore never touches the engine: register a producer in OD1's `OPPORTUNITY_SOURCES`, or (future, gated) adapt confirmed EL1 signals.
...
- **The ambient seam never touches the LLM.** Notices are fetched by a dedicated request outside the per-turn pipeline (CPA1 §5.2), voiced deterministically by the Behaviour Engine, and rendered as cards. No model call, no prompt bytes, no grounding.
- **The Notice Engine never serialises prompt content.** If notices are ever to reach the model as grounding — e.g. so the Companion can say "I mentioned your empty Thursday earlier" — they enter **only** as a capability Full Result composed by the Context Composition Engine under its budget, its guarantees, and its Context View contract. The Notice Engine emitting one byte of CONTEXT DATA would make it a second owner of the prompt — exactly what INT17 §7 forbids on its side and this document forbids on this side.
...
CPA1 remains the governing frame: one Companion, one Behaviour Engine, one Notice Engine, phrasing after selection, both seams ending in the same voice. This document adds component depth — the source taxonomy (§2), the ownership walk (§3), the graduation ladder (§4), and the rollout (§7) — and changes none of CPA1's rules. EL1 bounds the engine from the other side: a notice is *what we noticed*, learning is *what repeated and was confirmed*, and the two vocabularies stay deliberately distinct so neither can impersonate the other.
...
1. **Cap.** At most `MAX_NOTICES_PER_MOMENT = 2` notices per request. A moment is a panel-open, not a conversation turn.
2. **Priority.** High before medium before low, with the producer's own priority copied verbatim — the engine never re-derives it.
3. **De-duplication.** One notice per id; the same fact arriving via two categories is surfaced once.
...
5. **Honest absence.** No producer data → no notice → an empty set. The engine never pads, never invents a "tip of the day".
...
Raising the cap is the Notice Engine's equivalent of raising `CAPABILITY_CONTEXT_BUDGET_CHARS` — the move INT17 §7 forbids as a fix. More attention spent on the same selection buys nothing and costs every user; improvement must come from better producers and better relevance upstream, not a louder channel.
...
- **Any second ambient-notice channel** — a route, panel, or prompt block that surfaces unprompted facts without passing OD1 governance and the Silence Rules — **stop.** That is a second Notice Engine wearing different clothes; §7.2's channels are grandfathered *only* until their scheduled convergence.
...
- **Any notice that executes, or lowers a confirmation tier** — **stop.** Acting is always a separately confirmed intent; accepting an opportunity only ever writes delivery status.
- **Any persistence of a notice or a producer's fact** by this layer — **stop.** The only persistent facts are OD1's delivery lifecycle and (if ever approved) a delivery log — never content.
- **Any preference or behaviour change inferred from a single notice** — **stop.** Learning is EL1's, behind its thresholds and the household's explicit confirmation.
- **Any prompt bytes emitted by the Notice Engine** — **stop.** Grounding is the Context Composition Engine's, alone (§5.3).
- **Any new notice category without a registered owner behind it, or any guessed mapping for an unmapped domain** — **stop.** Unmapped is dropped, honestly.
- **Any raising of the per-moment cap presented as a fix** — **stop.** Improve the producers, not the volume.
- **Any second Notice Engine, anywhere, rather than the one extended in place** — **stop.**
...
| Requirement | Met by |
|---|---|
| One canonical Notice Engine defined | §0 mandate; §3 (single owner of the attention budget); §9 (second-engine stop) |
| What is noticed | §2 — closed seven-category taxonomy, one owner per source, never-noticed list |
| Who owns notices | §3 — full ownership walk; engine owns a decision, not data |
| How notices become recommendations | §4 — graduation ladder through OD1, Intent Engine confirmation, EL1 learning |
...
| Clear rollout path | §8 — NTC-P1…P6, each separately gated, activation before convergence |
