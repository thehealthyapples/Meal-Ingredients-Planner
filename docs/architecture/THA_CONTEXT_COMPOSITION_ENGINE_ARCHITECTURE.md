...
**Implementation record:** `docs/implementation/INT17_CONTEXT_COMPOSITION_ENGINE.md` (design + first measurement), `docs/implementation/INT18_CONTEXT_COMPOSITION_ENGINE_IMPLEMENTATION.md` (promotion to canonical + independent re-verification)
...
**Rollback:** `int17-rollback-20260708`, `int18-rollback-20260708`
...
A `ContextViewSpec` is a *statement about redundancy and constraint in a known payload shape*. It is **not** a business rule: it cannot add a fact, reorder a capability's ranking, or change what any field means. Two specs exist today (`profile:read`, `food-intelligence:report`), each justified in-code against the module that owns the payload.
...
1. **Migrate capabilities to own their Context View** (§2.1). The seam exists and the engine is indifferent. Twenty-one capabilities still use the generic derivation. Each migration is a capability-owned workstream.
...
7. **The guaranteed core is spent per group, so a capability with many groups can crowd out a capability with few.** Measured on CB-022 ("Which meals need better ingredient or nutrition data?"): `nutrition-knowledge:read scope=foods` — the entire 611-food registry, 40 groups — seats eight core items, leaving `meals` two, and the two meals that answer the question (`tuna spaghetti` and `Beef Concarne`, both `ingredientCount: 0`) are not among them. The engine cannot know that a registry dump is reference rather than evidence. The fix is a `ContextViewSpec` for `nutrition-knowledge:read scope=foods` (or a resolver that does not route it here), not a change to the balance rule. See `INT17` §7.
