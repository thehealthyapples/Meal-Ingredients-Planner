...
3. **No key space changes.** `weekNumber` stays 1–6. `dayOfWeek` stays `0 = Sunday`. `weekName` is untouched. **The Planner API is byte-identical** (**HT8**).
   ...
   > **AMENDED 2026-07-22 (`PLANNER1` — the Continuous Timeline), by the governing decision recorded in `docs/implementation/PLANNER_CONTINUOUS_TIMELINE.md` and approved before implementation.** This principle described the `CONV1` convergence, which changed no key space and had no need to. **`PLANNER1` is the change that makes the "`weekNumber` stays 1–6" clause false for `weekNumber` only, so `PLANNER1` amends it rather than quietly contradicting it** (CP4). **`dayOfWeek`, `weekName`, and the byte-identical existing Planner API are untouched — the amendment is to `weekNumber`'s *bound*, and to nothing else.**
   >
   ...
   >
   > **Why it had to give.** The 1–6 bound was the whole of the "six-week window" — a fixed rota that `TIME1 §6.3` had already recorded as **silently expiring** (`resolvePlannerWeek → window-expired`), with *"what the Planner should DO about it"* deliberately left open at `TIME1 §15.1`. **`PLANNER1` makes that product decision** (`TIME1 §15.1`, resolved; full design in `docs/implementation/PLANNER_CONTINUOUS_TIMELINE.md`): the timeline is continuous and the window never expires, because the current dated week is **created on demand** the moment a household arrives — the same honest, present-tense anchoring `createPlannerWeeks` already performs (**HT7**), never a back-fill of a past one. **Nothing about HT7 gives:** the 192 of 195 households whose weeks predate the anchor keep every `NULL` forever; those weeks become **undated legacy weeks**, preserved and reachable, never dated by guess.
