...
**Implementation status:** **DECLARED, BUILT, AND CONVERGING** — see § 17. The module (`shared/time/household-time.ts`) and `households.timeZone` exist as of 2026-07-17 (`CONV1 P5`); **Phase 3's T2/T3 consumers converged on 2026-07-17 (`CONV1 P6`)** — the Companion's anchor, the freezer, the diary, and the four `getGreeting()` copies now read the owner. **`planner_weeks.weekStartDate` exists as of 2026-07-17 (`CONV1 P7` / `SCH-2`) — both facts are built and the declaration is fully discharged**; it is written only at week creation and never back-filled (**HT7**), so every week created before that date is honestly unanchored, forever. **`CONV1 P8` converged the T5 consumers on 2026-07-17**: the five rival "current weeks" are one (`READ-3`), Home reads no device state (`OWN-6`), every Home card resolves from one canonical planner state (`BEH-3`), and the door is HOME2's resolver's (`BEH-9`). **The convergence's honest result is that 192 of 195 households resolve to `anchored: false` — permanently (HT7) — and are told so** (§ 13.1, as amended). **`CONV1 P9` retired `approxDate` the same day** — both copies of the fabricated-date builder are deleted, so **no date in THA is invented from a week number any more**. Streaks and savings are NOT converged (`user_streaks`' sixth private week). *(This line previously read "`planner_weeks.weekStartDate` still does not exist (Phase 4)" — corrected in the same change that made it false.)*
...
1. **The old behaviour is the floor — unless the old behaviour was the fabrication.** `resolvePlannerWeek → anchored: false` means the caller keeps **exactly today's behaviour**, *where today's behaviour is a truthful thing to keep*. An unanchored household gets what it gets today; an anchored one gets the truth. **Nothing correct regresses, ever.** Totality (**HT6**) *is* the compatibility strategy.

   > **AMENDED 2026-07-17 (`CONV1 P8`), by the governing decision recorded in that phase's report § 3.** This principle previously read *"the caller keeps **exactly today's behaviour** … **Nothing regresses, ever**"*, without qualification. **`CONV1 P8` is the change that made it false, so `CONV1 P8` amends it rather than quietly contradicting it** (CP4).
   >
   > **Why it had to give.** The floor was written to stop a convergence blanking the product (*"if `anchored: false` meant 'show nothing', the Foundation would blank the product for every existing household"*), and that concern stands. But it assumed the old behaviour was **correct behaviour worth protecting**. On Home it was not: Home rendered Week 1's meals beside Week 6's plant count — **two rival weeks, one screen, one paint** (`BEH-3`) — and *neither* number was a fact about the household (`HOME3` § 4 refused both). **Held literally, this principle protected a self-contradiction and made `BEH-3` unfixable**, because ending a contradiction necessarily changes one of the two contradicting sides. It was also already in tension with **`OWN-6`**, independently: Home's "today's behaviour" for meals *was* `localStorage`, and `OWN-6` retires it — you cannot retire the floor and stand on it.
   >
   > **The amendment, stated as a rule.** When a consumer's pre-convergence behaviour was **itself an invented answer** — a rival week, a fabricated date, a guess indistinguishable from knowledge — the floor does not protect it. The consumer states the absence honestly instead. **This is not licence to blank a surface**: an honest state is a *rendered, calm, first-class state* that says what THA does and does not know and keeps every door open. It is not an error, not a spinner, and not an empty state. **And it is never an excuse to pick a week** (`BEH-3`: *"do not pick a week to fix it"*) — the whole reason the floor cannot be a fallback here is that there is no honest fallback to be had.
   >
   > **Its scope is exact.** For the **192 of 195 households** whose weeks predate the anchor and can never be back-filled (**HT7**), `anchored: false` is not a transitional state — it is **permanent**, and the expected first-run experience until a canonical anchor exists. The only route out is the household **declaring** their week (`TIME1` § 6.2) — a governed act, not built. **A consumer whose old behaviour was correct still keeps it**: that is why the freezer, the diary and the greeting were converged under the unamended principle in `P6` and are untouched here.
...
3. **No key space changes.** `weekNumber` stays 1–6. `dayOfWeek` stays `0 = Sunday`. `weekName` is untouched. **The Planner API is byte-identical** (**HT8**).
   ...
   > **AMENDED 2026-07-22 (`PLANNER1` — the Continuous Timeline), by the governing decision recorded in `docs/implementation/planner/PLANNER_CONTINUOUS_TIMELINE.md` and approved before implementation.** This principle described the `CONV1` convergence, which changed no key space and had no need to. **`PLANNER1` is the change that makes the "`weekNumber` stays 1–6" clause false for `weekNumber` only, so `PLANNER1` amends it rather than quietly contradicting it** (CP4). **`dayOfWeek`, `weekName`, and the byte-identical existing Planner API are untouched — the amendment is to `weekNumber`'s *bound*, and to nothing else.**
   >
   > **What changed, stated as the new rule.** `weekNumber` is no longer a **fixed 1–6 slot label**; it is an **unbounded, monotonically-assigned, per-household ordinal** — a stable identity for a row, still `UNIQUE(user_id, week_number)`, still **never renumbered** (**HT8** stands: renumbering an existing week still silently rotates its consumers and is still forbidden). New weeks take `MAX(week_number) + 1`. **The week's *calendar coordinate* is, and always was, `weekStartDate` — never the number** (this is exactly BEH-5's *"the planner's `weekNumber` is a SLOT LABEL, not a time coordinate"*, now made structural: the number orders nothing in time, the anchor orders everything). Ordering, navigation and "which week is this" resolve over `weekStartDate`; `weekNumber` is an id the existing API keys on and keeps.
   >
   > **Why it had to give.** The 1–6 bound was the whole of the "six-week window" — a fixed rota that `TIME1 §6.3` had already recorded as **silently expiring** (`resolvePlannerWeek → window-expired`), with *"what the Planner should DO about it"* deliberately left open at `TIME1 §15.1`. **`PLANNER1` makes that product decision** (`TIME1 §15.1`, resolved; full design in `docs/implementation/planner/PLANNER_CONTINUOUS_TIMELINE.md`): the timeline is continuous and the window never expires, because the current dated week is **created on demand** the moment a household arrives — the same honest, present-tense anchoring `createPlannerWeeks` already performs (**HT7**), never a back-fill of a past one. **Nothing about HT7 gives:** the 192 of 195 households whose weeks predate the anchor keep every `NULL` forever; those weeks become **undated legacy weeks**, preserved and reachable, never dated by guess.
...
|---|---|---|---|---|
| 1 | **Rival "current week" implementations** — three server-side `max(weekNumber)` variants, the dashboard's `plannerFull[0]`, and Home's `localStorage` active-week | **5 → 1** | 5 | ✅ **DONE** — `CONV1 P8`. All five ask `server/lib/household-planner-week.ts`. Gated by `ht-one-planner-week-owner`, which **fails when a sixth is written** and is mutation-tested. *Note: `buildHouseholdHistory`'s `maxWeek` survives and is excised from that gate BY NAME — it orders the rota to fabricate a date and is target 5's (`BEH-5`, Phase 6), not a rival current week* |
| 2 | **Week-shape declarations** — Sunday-first arrays, Monday-first arrays, and the `[1,2,3,4,5,6,0]` reorder map | **19 across 16 files → 1** | 1, 3, 5 | **live, and begun** — `CONV1 P8` gave `MONDAY_FIRST_ORDER` its **first consumer** (the Dashboard), which *previously read "declared and unconsumed"*. That convergence **fixed a live off-by-one**: Monday-first labels indexed positionally over Sunday-first data rendered **Sunday under "Mon"** for every household (TIME1 § 3.2 — reported and unfixed since). The planner page still declares its own copy; the remaining rivals are unconverged |
...
| 5 | **Fabricated-date builders** — duplicated despite a docblock stating the extraction exists to prevent it | **2 → 0** | 6 | ✅ **DONE — `CONV1 P9`, 2026-07-17. Both copies deleted.** The private closure in `routes.ts` is gone (it imports the one owner); `server/lib/household-history.ts` no longer invents a date and **reads no clock**. A planner entry's date is now a **lookup** over `planner_weeks.weekStartDate` + `MONDAY_FIRST_ORDER`, or `null` — and `MealEntry.date` is nullable so an undated entry is representable rather than something the route layer must invent. **Measured, live: the retired builder reported the weekday of the REQUEST** — `(now.getDay() + dayOfWeek + 1) mod 7` — so one household's unchanged plan produced seven different "traditions" across seven days. Gated by `ht-no-fabricated-dates` + `ht-stories-tell-no-undated-story`, both mutation-tested |
...
>
> **As of `CONV1 P7` (2026-07-17), not one live entry on this list is blocked by a missing fact.** Both
> facts exist (§ 17). Targets 1, 2, 5, 6, 9 and 10 were live because **no consumer had been converged onto
> them yet**, which is Phase 5/6 work — not because the platform was unable to.
>
> **`CONV1 P8` (2026-07-17) discharged target 1 and began target 2; `CONV1 P9` discharged target 5
> the same day.** **Every fabricated date in THA is now gone**, and with it the last item on this
> list that told a household something untrue. What remains is target 2's remainder, target 6 (the
> Monday-week implementations, including `user_streaks`' sixth private week), target 9 and target 10
> — **none of which invents a fact; each is a duplicate of one.**
>
> **And P8 found the cost of `HT7` in the open:** **192 of THA's 195 households resolve to
> `anchored: false` permanently**, because their weeks were created before the anchor existed and are
> never back-filled. So target 1 is converged and **almost no household sees a different number** — they
> see an honest statement instead (§ 13.1, as amended). **The convergence is real; the truth it delivers
> is "THA does not know", and the only route out is the household declaring their week** (TIME1 § 6.2 —
> an extension point, not built).
...
| **`planner_weeks.weekStartDate`** | ✅ **BUILT — 2026-07-17** (`CONV1 P7` / `SCH-2`). Phase 4. Nullable, additive, **no SQL default, no row rewritten**. Written **only** by `storage.createPlannerWeeks` — the Planner's existing single write funnel, no new writer — as `mondayOf(householdToday(now, zone)) + 7 × (N − 1)`, because the moment the six slots are made consecutive is the only moment THA can honestly know what they mean (**HT7**; TIME1 § 6.2). **NEVER back-filled (HT7):** the 1,152 planner weeks that existed on the day it landed hold `NULL` and keep it **forever**, so `resolvePlannerWeek` still answers `anchored: false` for every household that was already planning — the honest floor (§ 13.1), not a defect and not a backlog. *This row previously read "**Does not exist.** Phase 4" — corrected in the same change that made it false* |
...
| **The T5 consumers** | ✅ **CONVERGED — 2026-07-17** (`CONV1 P8`). Phase 5. The **five rival "current weeks" are one**: `server/lib/household-planner-week.ts` over `resolvePlannerWeek`, published once at `GET /api/planner/current-week` and read by Home and the Dashboard through one hook. Retired: three server `max(weekNumber)` variants (**≡ the constant 6**), the dashboard's `plannerFull[0]` (**≡ 1**) and Home's `localStorage` (**≡ 1**). **`BEH-3` is closed** — Home's two cards read one field and cannot disagree; **`OWN-6` is closed** — no device decides a household's week; **`BEH-9` is closed** — the door is HOME2's resolver's, its first production consumer. **Streaks and savings are NOT converged** (`user_streaks.weekStartDate` — a sixth private week; `OWN-2`/`BEH-9`'s siblings), **`approxDate` is retired (Phase 6 / `CONV1 P9`, 2026-07-17).** *This row previously read "Still read their own private clocks — but the excuse is gone" — corrected in the same change that made it false* |
