...
| **4 — Build Preparation Knowledge (WS5A)** ✅ **BUILT (`PHASE5A`, 2026-07-12)** | The first knowledge type built *after* this document exists — the first real test of whether §1/§3/§4/§5 as written are sufficient guidance without further re-derivation | **PASSED.** It ships on the existing Nutrition Facts pipeline row (§1.1) with **zero new lifecycle invented**: the same `isEvidenceBackedClaim` gate, the same `sourceRefs`/`reviewedAt`/`reviewedBy` columns, the same single writer (`seed-knowledge-registry.ts`), the same human confirm gate (`signoff-knowledge-claims.ts`, extended by one edge), the same key space (`knowledge_foods.slug`). **The one deliberate divergence from WS5A §3.1** — its proposed bespoke five-state `editorialStatus` column — was *rejected precisely because* Rule KC1 forbids it: that column predates PKC0/KNOW5, which made the evidence chain itself the render gate, and adding it would have created a second lifecycle for one fact. Coverage: 39 preparations, 420 links, **0 effects** — existence is stated freely, effect defaults to silence |
...
| **7 — Populate Product Knowledge (added `PKR3`)** 🟡 **POPULATED (`PDA1`) + READ PATH BUILT (`PHASE5A`)** | The **Platform Discovery & Experience Audit** discovers every surface, journey, capability, hidden experience, claim and asset THA ships; each finding lands as an owned entry in the Product Knowledge Registry, or is explicitly declined (Rule KC12). Built **with** its validator, not before it (§4.3) | **Populated:** 154 entries, each with a named owner and a deliberate visibility; the prose↔inventory map is **bijective and enforced** (`scripts/verify-product-inventory.ts`). **Read path (`PHASE5A`):** the `product-knowledge` capability is registered and bound, so the Companion can now *query* the registry rather than know nothing (Rules PKR20/PKR21); visibility filters **before** composition and **fails closed** (PKR22/PKR26). **STILL OPEN, and not claimed otherwise:** (a) **currency is unenforced** — nothing checks that `last_verified` is inside a bar or that an entry's `sources` still resolve, which is Risk R8 exactly as §4.3 predicted; (b) **Rule PKR27 is unaudited** — the Companion now *can* query the registry, but no one has yet swept the existing prompts, templates and fallback strings for sentences about THA that duplicate it. The second narrator is now *avoidable*; it has not been shown to be *absent* |
