...
**There is one Behaviour Engine. Every word the Companion says to the user — a grounded answer's tone, an honest gap's phrasing, a guidance suggestion's label and order, a greeting, a celebration, a growth statement, a voiced observation — passes through it as the last transform before rendering. It is the single owner of voice, tone, personality, and (future) language of expression. No other component may reword, relabel, reorder-for-tone, or restyle what the platform has already decided to say.**
...
| Observation Engine (INT20) | *Which* true facts reach the **user** unprompted, under one attention budget | platform → user (unprompted) |
...
| Voiced observations | `phraseObservation` — dispatches each Observation's fact kind to the existing builders above; producer content verbatim-or-prefixed, never reworded | dormant until OBS-P1 |
...
- **Selection and attention.** Which facts are surfaced, and how many, is the Observation Engine's (Silence Rules) and the producers'. Day-seeded phrasing varies the *voice*, never the *selection* (INT20 §6.6).
...
```
   ...
   (turn-fallback state + its disclosure inputs · an eligible
    suggestion set · a GrowthSignal · an Observation · a
...
- **Dispatch, never rewording.** `phraseObservation` routes each fact kind to an existing builder (`growth` → `phraseGrowth`, `streak`/`diversity` → `buildCelebration`, `opportunity`/`seasonal` → `voiceGuidanceLabel`). Content produced by a capability — an opportunity's `explanation` and `suggestedAction` — is never reworded, only optionally prefixed. Verbatim-or-prefixed is the observation rule; verbatim-or-absent is the producer rule (INT20 §9); both hold here.
...
1. **Deterministic template voicing** (server-side, no model): fallbacks, labels, greetings, celebrations, growth, observations. The words are fully determined by `(fact, personality, seed)` — auditable string-for-string in tests.
...
- **Every seam, same voice.** The per-turn voice seam and the ambient observation seam terminate in the same engine (CPA1 §2 names this structural). A user who chose `coach` hears `coach` in the answer, the gap, the suggestion label, the greeting, and the notice. There is no surface with its own voice.
...
### 7.2 With the Observation Engine (INT20)

**Selection, then phrasing — in that order, with nothing in between.** The Observation Engine's Silence Rules decide *which* facts (at most two) reach the user; `phraseObservation` decides the *words*, per the user's voice. The engine pair shares one fact shape (`Observation`), one direction of flow, and the verbatim discipline: producer `explanation`/`suggestedAction` cross both engines unreworded (optionally prefixed at the voice seam, exactly like guidance labels). Day-seeded phrasing variety never feeds back into selection — a fact is not resurfaced because its wording changed. When OBS-P1 wires the observation route, the route composes the two engines in this order; any other composition (voicing before selection, a second phraser beside the panel) is a violation of both documents.
...
2. **Two exports are code-complete and dormant.** `phraseGrowth` and `phraseObservation` have no live route on this branch — the same dormancy INT20 §7 records for the observation chain; OBS-P1 activates both together. `buildGreeting`/`buildCelebration` are called only via `phraseObservation`; `FloatingAssistant.tsx`'s empty-state greeting remains hardcoded (CPA1 G5).
...
**BEH-P2 — Voice the activated observation seam** (jointly with OBS-P1, which owns the route). *Exit: every ambient notice reaches the user through `phraseObservation`; no notice text exists outside the registry.*
...
| Integration with CCE and Observation Engine | §7.1 / §7.2 (+ §7.3 TIP1–3/CPA1) |
