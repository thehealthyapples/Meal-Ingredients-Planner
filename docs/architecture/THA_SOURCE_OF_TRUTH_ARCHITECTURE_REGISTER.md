...
>
> **The season is not Household Time** (`HT17`): `shared/time/household-time.ts` supplies this rule's **input** (a civil date) and never its **answer**. One process-local frame read survives — the named transitional adapter `seasonOfLocalDate()` — which preserves today's behaviour exactly and is **Phase 3's** retirement target (`CONV1 P6`), not this phase's.
