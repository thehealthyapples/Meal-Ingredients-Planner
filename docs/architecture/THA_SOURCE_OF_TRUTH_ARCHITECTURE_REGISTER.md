...
| Consumers | `pantry-page.tsx`; `shopping-workspace-page.tsx:1564` (**SHOP3**: the second Shopping consumer, `shopping-list-page.tsx`, was retired — Shopping now reads the Pantry from one place); `PantryKnowledgeHub.tsx`; `PantryIntelligencePanel.tsx` (*"It OWNS NOTHING"* — `:8`); capabilities `pantry` (`capability-registry.ts:529`) and `pantry-discovery` (`:615`); `pantry-intelligence-assembler.ts:140`; `opportunity-engine.ts:400`; `planner-explanation-context.ts:227` |
