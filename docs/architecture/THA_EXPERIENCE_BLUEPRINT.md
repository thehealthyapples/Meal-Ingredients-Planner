...
|---|---|---|---|
| **E3** | **The open view** | The orchard visible as itself, generously; sparse content on its ground; the view *is* part of the room's purpose. At Home it *may* be composed as a **plaster architectural aperture — an arch** (rule 4) | **Home only** |
...
4. **An architectural aperture is not a drawn frame.** *(Amended `ARRIVAL1`, 2026-07-17.)* Home's E3 open view **may** be composed as a **plaster architectural aperture — an arch cut into the wall, with a soft plaster reveal and no hard drawn edge**. E2's clause *"framed by composition, never by a drawn frame"* stands unchanged for every E2 room, and its intent is preserved here: what it forbids is the **decorative picture-frame** — the orchard mounted on the wall as a framed print, a rectangle of moulding drawn *around* an image that is otherwise wallpaper. An **opening in the wall** is the opposite thing: the orchard is seen *through* the architecture, not hung *on* it, exactly as a real window or archway frames a real view (the tension NORTH4 § 4.1 named — *"architecture frames views with drawn frames; that is what a window is"* — resolved in the orchard's favour, for Home only). The line, testable: a frame you could lift off the wall and hang elsewhere is decorative and forbidden; a reveal the wall turns into, that the orchard is genuinely behind, is architecture and admitted. The aperture never carries type (§ 6.1 stands), never animates (§ 6.1 stands), and remains a per-domain constant at E3 (rule 1) — the arch is a *composition* of the one E3 exposure, not a new level. Home only; no E2 room gains an arch by this rule.
...
2. **Data-borne or dead.** Every detail renders something TRUE from a canonical owner — the clock, the plan, cook counts, list state, freshness data, plant counts, diary history, analysis history, household membership. A painted prop (drawn fruit, fake steam, decorative crumbs) is fabricated feeling, forbidden by construction. *(Refined by `ED1` / `LIVINGHOME2`, 2026-07-22: the prop ban is a ban on a **claim without data** — fabricated feeling that pretends to be the household's own life. A registered **Environmental Dressing** item is the distinct, lawful class of a **claim-free object of the home's own hospitality** (`LIVING_HOME_ENVIRONMENTAL_DRESSING_ARCHITECTURE.md`, ED1–ED12): it asserts nothing about any household, so it cannot fabricate their feeling. This law's purpose — no fake life, no fabricated household truth — is untouched and binds the **Life** register in full; what changes is only that claim-free warmth owned by the home gains a name instead of being mistaken for a prop. Environmental Dressing is **DECLARED-NOT-BUILT**; no dressing item appears until its own phase gates pass.)*
