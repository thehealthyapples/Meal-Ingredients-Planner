...
**Current behaviour**
Resolved by **commit `d3c8827`** (current HEAD) and documented in `docs/implementation/ADMIN1B_ADMIN_DOMAIN_ENTRY_POINT.md`. All seven admin routes are now registered in `App.tsx:210–216`, and both dropdown menus consolidate to a single **Admin → `/admin`** entry (admins only). The hub (`admin-page.tsx`) surfaces all sub-sections as cards: Users, Picks, Recipe Sources, Companion Intelligence, Benchmark Households, Intelligence Dashboard. Verified live: `/api/admin/*` endpoints return `200`/`403` (healthy + gated).
...
- **Admin domain (the sanctioned restore target):** already restored by commit `d3c8827` (current HEAD) and documented in `docs/implementation/ADMIN1B_ADMIN_DOMAIN_ENTRY_POINT.md`. Re-implementing would be redundant. Verified reachable and healthy. Residual polish is captured as F3/F5 for a follow-up, not done here.
