...
`docs/implementation/PROD1_PRODUCT_COMPLETION_PROGRAMME.md` already exists and carries almost the same title. **PROD2 is its successor, not a second owner of the same question.** PROD1 closed one theme — *rooms that presented a failed load as an absence* — and closed it completely, then listed **14 remaining product gaps** in its §8, sorted by what each was waiting on. PROD2 is the decision-free remainder of that list, joined with the implementation backlog of `docs/investigations/LAUNCH1_THA_LAUNCH_READINESS_AUDIT.md`.
...
- Backlog: `docs/investigations/LAUNCH1_THA_LAUNCH_READINESS_AUDIT.md` (§2.1 gates, §3 area findings, §5 top-20, §6 Path B withdrawal list, §7 order)
- Predecessor: `docs/implementation/PROD1_PRODUCT_COMPLETION_PROGRAMME.md` §8 (the 14 remaining gaps)
