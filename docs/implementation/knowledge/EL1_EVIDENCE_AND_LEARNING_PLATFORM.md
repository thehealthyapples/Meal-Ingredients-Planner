...
- [x] `docs/implementation/intelligence/OD1_OPPORTUNITY_DELIVERY_FRAMEWORK.md` (the exact Port → Handler → Binding shape, `IStore`/`Database`/`InMemory` store discipline, and pure-core/I-O-orchestration-in-one-framework-file split this task mirrors verb-for-verb and file-for-file)
- [x] `docs/implementation/intelligence/INT35B_COMPANION_LEARNING_AND_OBSERVABILITY.md` and `server/intelligence/conversation/companion-learning-store.ts` (confirmed this is a DIFFERENT concern — meta-learning about the Companion's OWN routing/matcher performance from a privacy-safe miss log, never household outcomes or preferences. No overlap with EL1's scope; the `reviewRecommendation` "writes only status/reviewedBy/reviewedAt" hard rule is the discipline this task's own `confirmSignal` mirrors.)
...
- This implementation record exists at `docs/implementation/knowledge/EL1_EVIDENCE_AND_LEARNING_PLATFORM.md` ✅
