...
- [x] Read `docs/implementation/INT11_CAPABILITY_CARDS_SPECIFICATION.md` in full — confirmed all six Capability Cards are `STATUS: COMPLETE`, each grounded in file:line evidence, each with its own "OPEN DECISION" lines that this workstream must carry forward unresolved.
...
The original `docs/implementation/INT11_CAPABILITY_CARDS_SPECIFICATION.md` report was **not modified** — it remains as the historical investigation record (evidence + methodology), per the "preserve all existing evidence" instruction. It is no longer the canonical source for the card content itself; the registry and README now point readers to the canonical documents instead.
...
| `docs/implementation/EPIC_1_5_CAPABILITY_ARCHITECTURE_PROMOTION_IMPLEMENTATION.md` | **Created** — this report |
