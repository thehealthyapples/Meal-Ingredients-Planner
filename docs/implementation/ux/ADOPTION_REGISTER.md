...
| Signature typography (the third voice) | `NORTH1 (2026-07-17) — ADOPT is executed on `/home`, and the row **does not close**, because the prototypes it names are still in the tree. This row's own rule is that it "closes when the prototypes are deleted, not when the decision is minuted" — and eight of them are still here.` | **Delete the eight arrival prototypes, after CONV1 P6 lands.** `pages/dev/arrival-{a-welcome,b-orchard,c-workspace,d-quiet,e-restraint,experience,s1-quiet,s2-walking-home}.tsx`, plus `pages/dev/exp2-shared.tsx` (their shared module, including `useSignatureFont` — now dead, since index.css loads the webfont), their `App.tsx` lazy-imports and routes, and the four `scripts/capture-arrival-*.ts` / `capture-exp{2,3}-*.ts` harnesses that target them. Three of the eight (`arrival-experience`, `arrival-a-welcome`, `arrival-s1-quiet`) consume `.text-signature` and are the reason this row stays open; all eight are dev-only (`import.meta.env.DEV`), so no household downloads any of them and nothing a household can see is affected by the delay. The trigger is CONV1 P6 landing, and the deletion is then mechanical. | Colin Clapson — the deletion is blocked on a concurrent session, not on a decision | **The deletion was attempted and deliberately abandoned, and that is the honest state.** `pages/dev/arrival-a-welcome.tsx` and `pages/dev/arrival-s1-quiet.tsx` — two of the eight — are being edited RIGHT NOW by the concurrent CONV1 P6 session, which is converging THA's four `getGreeting()` copies into the new `client/src/lib/greeting.ts` (Household Time architecture § 14, target 3: 4 → 1). Both files carry uncommitted work importing that new module. Deleting them would have destroyed another session's in-flight change AND silently altered its arithmetic underneath it — two of its four consumers vanishing mid-convergence. So NORTH1 deleted nothing it did not own: `material-a-warm-layers.tsx` (untouched by P6, and its trigger fired today) is gone, and the eight arrival prototypes are not. **This is recorded rather than quietly deferred**, because an ADOPT executed on the live surface while three prototypes still reach `.text-signature` is exactly the authored-but-unadopted-successor state this register exists to make impossible to hide — and it is now a state with a named cause and a named trigger instead of a silence. |
