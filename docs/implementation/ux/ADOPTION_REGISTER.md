...
| Loading state | `<Loader2>` usages | 180 | Capped, not banned: 140 are the exempt inline pending mark above. The population may fall, never rise. |
| Mutation feedback | Raw `useMutation` — feedback owned by the call site | 138 | Not all are defects: a mutation with no household-visible outcome owes no toast. Capped so the population cannot grow. |
...
|---|---|---|---|
| `client/src/lib/companion-delight.ts` | 71 | Dead module — all five exports unconsumed. PX1 §5 recorded that `prefersReducedMotion()` had 0 consumers; PX1-W1.4 answered by making `variantForInteraction()` (in the same file) call it — but **nothing imports the module at all**, so W1 connected one dead function to another. Recorded here as the correction. The reduced-motion *guarantee* is unaffected and live: it is delivered by the `@media (prefers-reduced-motion: reduce)` block and framer-motion's `MotionConfig` (see the `motion` concern), neither of which touches this file. | Colin Clapson — delete, or adopt in the Companion's motion surfaces |
