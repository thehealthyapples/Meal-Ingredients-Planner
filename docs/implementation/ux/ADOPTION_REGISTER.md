...
| Signature typography (the third voice) | Every household-facing surface, including the live Home (`/home`) | Not an exemption from the owner — an exemption from USING it. No household-facing surface has yet adopted the signature voice. UXHOME1 amended the law and built the prototype and deliberately did NOT adopt it into the live Home in the same change; adoption was a separate, explicit decision. **That decision is now recorded: ADOPT (Colin Clapson, 2026-07-17, via ODL2)** — see the open migration. This exemption stands only until the North Star Home workstream executes it, and then it goes, replaced by exactly one named permitted surface. |
...
|---|---|---|---|
| Theme / colour mode | `dark:` utilities authored across the client | 862 | 2026-07-17 |
...
| Signature typography (the third voice) | `ARRIVAL1 — the signature voice is admitted, and adopted by nothing a household can see. **Disposition recorded 2026-07-17 (ODL2): ADOPT.** The decision is closed; the execution is not.` | **The execution of ADOPT**, in one change, owned by the North Star Home workstream: fold the Arrival Experience greeting into home-experience-page.tsx; move the Caveat `@import` into client/src/index.css so the webfont is loaded in production for the first time; name `/home` as the ONE permitted surface in this row; and DELETE every exploration prototype — `/dev/arrival` (ARRIVAL1), the five EXP2 prototypes (`/dev/arrival-{a..e}-*`) and the two EXP3 synthesis candidates (`/dev/arrival-s1-quiet`, `/dev/arrival-s2-walking-home`) — with their routes. ODL1 built Home's greeting in the **display** voice as the lawful interim, so the upgrade is a one-line change to that greeting plus the deletions. **Three constraints ride on the execution.** (1) § 8's "a second surface does not follow from the first": `/home` is one surface, and Home's greeting is one moment — not Home's headings, labels or values. (2) Blueprint § 12.2 caps the Living Detail at **at most once per day**, and § 10 there holds the signature "arrival-only". (3) The webfont becomes a production download for the first time; it is a household's bandwidth, so it is subject to UIA § 15's floors and must not block the greeting's paint. **What may not happen is the third outcome**: the prototypes surviving their own decision. ADOPT was chosen; the deletions are not optional, and they are what closes this row. | Colin Clapson — ADOPT recorded 2026-07-17 (ODL2); execution belongs to the North Star Home workstream | A signature typeface is a brand decision, not an engineering one, and it is the kind of decision that is made by looking rather than by reading. The prototype existed to be looked at, and the owner has now ruled: **ADOPT**. This row was recorded on the day the prototype was built rather than after it was forgotten, because a prototype nobody decided about is how a third typeface enters a product by accident — the exact failure the original § 8 rule was written to prevent, and the one the amendment is only safe against if this row is honoured. **The row is not discharged by the decision.** ADOPT recorded and unexecuted is still an authored-but-unadopted successor sitting live-looking in the tree (PX1's PageHeader.tsx, exactly). It closes when the prototypes are deleted, not when the decision is minuted. ODL2 recorded the ruling and did not execute it, because execution edits Home and ODL2 was governance only. |
| Depth, warm light and materiality (the § 4 vocabulary) | `ODL2 (2026-07-17) — the vocabulary is admitted, valued and consumed, but its only consumer is a development-only reference.` | **Home's E3, and then a deletion.** `pages/dev/material-a-warm-layers.tsx` (`/dev/material-a-warm-layers`) is the vocabulary's **reference implementation** and its sole consumer: it holds these tokens honest, and it is the surface that proves them by being looked at. It is `import.meta.env.DEV` only, so no household downloads it. **Its deletion trigger is Home's adoption of the ground plane** — when Home stands at E3 on a real ground, this page and `scripts/capture-exp4-materiality-depth.ts` are deleted in that same change, completing the disposition EXP4 § 6 set for all three studies. **Why it was not deleted with B and C**: EXP4 § 6 assumed the ideas would graduate into a *language*, and they graduated into *tokens* — which need a consumer. Deleting A would have left twenty-one tokens defined and adopted by nothing: the authored-but-unadopted foundation this register exists to make impossible, and the precise state UIA § 17 says it exists to end. Keeping the winner as the reference until the real surface lands is the same shape this register already blesses for `signature-typography`, and it is recorded here with its trigger so it cannot outlive its purpose quietly. | Colin Clapson — with the North Star Home implementation, the first household surface that needs a ground | The vocabulary is real, valued and visible, and it is still adopted by nothing a household can reach — so it is recorded, not assumed. This row closes when Home consumes the tokens and the reference dies; it does not close when the values look right. **E2's exposure value and this vocabulary's dark resolution are the two things no surface can yet verify**: no E2 room exists, and THA's dark theme is unreachable (see `theme-colour-mode`). Both are reasoned decisions with their reasoning written down, sitting in one place, cheap to correct on the day something can finally show them wrong. |
