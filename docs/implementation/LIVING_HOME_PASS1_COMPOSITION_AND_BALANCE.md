...
- **Pre-state evidence:** `docs/investigations/HOMEOWNER2_LIVING_HOME_REVIEW.md` (the Home Owner's Apple-Design-Award-bar critique, 7/10) and `docs/implementation/LIVING_HOME_COMPLETION_PROGRAMME.md` (LHC1), whose roadmap items **E** (own the empty desktop half), **I** (Companion presence & focus), **J** (calm the Planner grid), **K** (spacing / faint controls), **L** (hide the machine email) and **M** (over-tabbed / spacing) this pass discharges the high-confidence slices of.
