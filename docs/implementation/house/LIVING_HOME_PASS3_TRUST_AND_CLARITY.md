...
- **Pre-state evidence:** `docs/investigations/house/HOMEOWNER2_LIVING_HOME_REVIEW.md` (the Home Owner's Apple-Design-Award-bar critique, 7/10 — which circled the Shopping "12 vs 3" and Nutrition "17 vs 18" counts as trust defects), `LIVING_HOME_COMPLETION_PROGRAMME.md` roadmap item **D** (reconcile on-screen counts), and `LIVING_HOME_PASS2_HOSPITALITY_AND_WARMTH.md` § Recommendations (which scoped this pass's seven Trust & Clarity candidates).
