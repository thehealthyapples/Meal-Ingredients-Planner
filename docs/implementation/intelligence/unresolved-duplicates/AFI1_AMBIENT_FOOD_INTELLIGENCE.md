...
| **Working-tree caveat** | The tree was **dirty at session start — NOT this session's changes.** ~132 pre-existing sibling-session edits **plus** FI20's own uncommitted Food-Comparison work (awaiting its own review). The tag protects **committed** state only. AFI1 left all of that byte-untouched and committed nothing. **Note:** a concurrent sibling process deleted two untracked docs mid-session (this file and FI20's); both were restored from context. A shared dirty tree is a real hazard — commit early once owner approves. |
...
`notice-engine` **65/65** — zero regression across delivery, ranking, attention
and Companion voicing.
...
| `meal-card-ambient-uplift.png` | Meal card | The **new scoped mount** on `meal-detail-page` (the Overnight Oats meal) — "A small lift for this week · 1", showing only the `planner-meal-uplift` card and no unrelated domain-mates. |
...
2. **Ingredient-aware matching (a visible quality gap — highest-value follow-on).** The generator matches the uplift engine on the meal *name* only (the planner port's cheap `getMeal`, `ingredients: []`). This can suggest an ingredient the meal already contains — the captured demo shows *"add chia seeds"* for an Overnight Oats meal whose ingredient list already includes chia seeds, because the rule fired on the name without seeing the ingredients. Feeding the meal's ingredient list (via the meal-intelligence read the meal page already performs) would suppress already-present suggestions and find better-targeted lifts — without any new knowledge.
