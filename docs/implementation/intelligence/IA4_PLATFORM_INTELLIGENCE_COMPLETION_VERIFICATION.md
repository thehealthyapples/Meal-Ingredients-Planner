...
**What was claimed.** `capability-registry.ts`'s own description of `food-intelligence` (unchanged since FI3): *"The foundation Domain Intelligence engine consumed by Companion today…"* `docs/implementation/intelligence/FI3_FOOD_INTELLIGENCE_ENGINE_FOUNDATION.md` line 7 makes the identical claim, then contradicts itself 300 lines later (line 375): *"Wire one real domain consumer… to prove the capability end-to-end for a real surface"* — named as **unstarted** future work in the same document.
