...
**Note on numbering:** `FI5` is already in use in this repo for a distinct, unrelated workstream (`docs/implementation/intelligence/FI5_FOOD_INTELLIGENCE_UI_ACTIVATION.md`, 2026-07-03). This is consistent with existing practice here — `FI1`, `FI2`, `FI3` and `FI4` are each already reused across multiple, differently-titled documents — so this document keeps the requested filename and is disambiguated by title, not number.
