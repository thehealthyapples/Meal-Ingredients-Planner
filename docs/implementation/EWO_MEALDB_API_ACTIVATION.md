...
| This task's writes | `server/lib/external-meal-service.ts`, `server/routes.ts`, `.env.example` (new), `docs/implementation/EWO_MEALDB_API_ACTIVATION.md` (new) |
...
- [x] `docs/investigations/FS1_THA_KNOWLEDGE_SOURCE_AND_LICENSING_AUDIT.md` (G2 — origin of this task: production use of TheMealDB's shared test key breaches TheMealDB's own published terms)
...
- **Files created:**
  - `.env.example`
  - `docs/implementation/EWO_MEALDB_API_ACTIVATION.md`
- **Rollback commands:**
  ```
