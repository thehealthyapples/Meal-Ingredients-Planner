...
- **a session secret that does not fail closed** — `secret: process.env.SESSION_SECRET || "r3pl1t_s3cr3t_k3y_123456"` (`server/auth.ts:41`). The fallback is committed to this repository. Anyone holding it can forge a session cookie for any account, including an admin account, if the environment variable is ever absent;
