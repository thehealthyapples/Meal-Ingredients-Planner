...
import { Plus, X, ShoppingBasket, Loader2, ChefHat, Leaf } from "lucide-react";
...
export default function QuickMealPage() {
  ...
  const createBasketMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "Basket list created", description: `${items.length} item${items.length === 1 ? "" : "s"} added to your basket.` });
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-6">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="text-xl font-semibold tracking-tight">Quick Meal</h1>
      </div>
      ...
      <Card className="mb-4">
        ...
        <CardContent className="space-y-3">
          ...
          {items.length === 0 && (
            <p className="text-xs text-muted-foreground text-center py-4">Add components above, then create a basket list.</p>
          )}
        </CardContent>
      </Card>
      ...
      <Button
        className="w-full"
        disabled={items.length === 0 || createBasketMutation.isPending}
