...
-   **Demo Mode Phase 1 — Temporary Demo User**: Clicking "Explore The Healthy Apples →" on the auth page calls `POST /api/demo/start`, which creates a real temporary user (`isDemo=true`, `demoExpiresAt=now+20min`, `onboardingCompleted=true`, `emailVerified=true`) in a transaction, seeds 4 demo meals + planner entries for week 1 + 12 shopping list items, logs them in through the normal session flow, and redirects to `/`. A global `DemoBanner` (green, turning amber under 2 min) appears in the `ProtectedRoute` shell with a live countdown driven by `user.demoExpiresAt`. Demo expiry is enforced server-side: a middleware in `setupAuth()` runs on all `/api/` routes and returns `{ code: "DEMO_EXPIRED" }` (HTTP 401) when expired; the queryClient intercepts this and redirects to `/auth?demo=expired`, which shows a friendly message. A `cleanupDemoUser(userId)` storage method explicitly deletes all demo user data in the correct order. Legacy static `/demo` routes remain isolated. Schema: `users.is_demo` (boolean, default false) + `users.demo_expires_at` (timestamptz). Migration: `2026-03-13_add_demo_user_fields`.
