...
-   **Pantry Staples**: Users maintain a list of ingredients they usually have at home (olive oil, herbs, spices) via the Profile page. Ingredients are grouped by Larder/Fridge/Freezer. The shopping list automatically collapses pantry items into a "Staples — usually in stock" section. A client-only "Need this week ↑" toggle promotes individual items back to the main list. Ingredient keys are normalised via `shared/normalize.ts`. See `docs/pantry-components-pairings.md`.
