...
const OUT = path.resolve("docs/implementation/household-evidence"); mkdirSync(OUT, { recursive: true });
...
async function run() {
  ...
  const page = await ctx.newPage(); await openPantry(page);
  await page.screenshot({ path: path.join(OUT, "arrival.png") }); console.log("arrival");
  for (const z of ["fruit-bowl", "fridge", "rootveg", "bread", "tea-coffee", "cupboard"]) {
