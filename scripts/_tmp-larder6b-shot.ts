...
const OUT = process.env.SHOT_OUT ?? "/tmp/claude-1000/-home-runner-workspace/84b09ef8-14a0-49af-8a46-6924c0254bbc/scratchpad/shots";
const BASE = process.env.UX_BASE_URL ?? "http://localhost:5000";
const SHOTS = [
  { label: "desktop", w: 1440, h: 900 },
  { label: "tall", w: 1440, h: 1500 },
...
(async () => {
  ...
  for (const s of SHOTS) {
    ...
    await p.screenshot({ path: `${OUT}/${s.label}.png`, fullPage: !!process.env.FULL });
    if (errs.length) console.log(`[${s.label}] errors:`, errs.slice(0, 6).join(" | "));
    await ctx.close();
  }
  await b.close();
  console.log("done");
