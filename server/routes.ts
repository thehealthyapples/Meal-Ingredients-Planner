...
// BENCHINT2 — meal derivation (nutrition + allergens) and the recipe-page extraction helpers it
// uses were route-local functions in this file. They are now services, so the ONE derivation path
// is reachable by every producer of a meal — this route, and the Benchmark World seeder — instead
// of being duplicated or silently skipped. Behaviour is unchanged: this is an extraction, not a
// rewrite. See docs/implementation/benchmarking/BENCHINT2_BENCHMARK_RUNTIME_CONVERGENCE.md.
import {
  autoAnalyzeMeal,
  forgetAnalyzedMeal,
  scrapeNutritionFromSource,
  cleanIngredientForLookup,
  isDescriptiveUnitIngredient,
  parseIngredientGrams,
  fallbackIngredientGrams,
} from "./services/meal-analysis";
import {
  isMacroLine,
    ...
    if (hasNoNutrition || isOldBuggyFormat || hasEstimatedAndSourceAvailable) {
      (async () => {
        try {
          ...
          if (isOldBuggyFormat) forgetAnalyzedMeal(meal.id);
