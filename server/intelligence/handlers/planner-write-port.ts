 ...
 * exactly (reused, not re-derived); `addPlannerEntry` is the one new write method,
 * the SAME call `POST /api/planner/days/:dayId/items` makes today.
 *
 * OWN-DATA ONLY BY CONSTRUCTION: ownership is verified by the HANDLER (replicating the
 * exact sequence the route uses) using these read methods before the write method is
